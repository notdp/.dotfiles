...
select: 1,2,3,4,5,6,8,9,10,13 / all / restore
```
...
| 9   | effort 级别  | `["off","low","medium","high"]` | 按 provider 区分 | +72~132 | KOH(+66) + $A(+6~66)               |
| 10  | 禁用内置模型 | `allowed:!0`     | `allowed:!1`     | 0    | validateModelAccess 非 custom 模型拒绝 |
| 13  | 禁用自动更新 | `let H,{remoteConfig:$}=...` | `return null;/*..*/` | 0 | checkForUpdates() 直接返回 null     |
| 补偿 | 死代码区域   | 多处死代码        | 注释/缩短填充     | 对应负值 | 统一补偿 mod9                       |
...
- mod9: 两处 custom model effort 列表: KOH(按 provider 区分) + $A(加 "max"/"xhigh")
...
mods/mod9_custom_effort_levels.py # custom model effort 级别 (+72~132 bytes)
mods/mod10_disable_builtin_models.py   # 禁用内置模型，只允许 BYOK (0 bytes)
mods/mod13_disable_auto_update.py      # 禁用自动更新 (0 bytes)
```

mod9 产生 +72~132 bytes，由 `comp_universal.py <bytes>` 统一补偿（脚本会输出实际字节数）。
...
用法：`python3 comp_universal.py <N>` 补偿 mod9 产生的 +N bytes。
...
python3 ~/.factory/skills/droid-bin-mod/scripts/mods/mod9_custom_effort_levels.py  # +N bytes (脚本输出实际值)
...
python3 ~/.factory/skills/droid-bin-mod/scripts/compensations/comp_universal.py <N>  # 补偿 mod9 的 +N bytes
