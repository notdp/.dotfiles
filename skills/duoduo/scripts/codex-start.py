#!/usr/bin/env python3
"""
codex-start.py <comment_id>
启动 Codex session 并发送审查指令
...
result = subprocess.run(
    [sys.executable, f"{S}/session-start.py", "codex", "gpt-5.2", PR_NUMBER],
...
你是 Codex (GPT-5.2)，duoduo review 流程中的审查者。
...
1. Send result to Orchestrator via FIFO (把完整评论内容发过去):
   $S/fifo-send.sh orchestrator {PR_NUMBER} "<CODEX>$REVIEW_CONTENT</CODEX>"
2. Post the review comment (UI)

## After completing Stage 1
完成后继续等待 Orchestrator 的后续指令（交叉确认等）。"""

subprocess.run([f"{S}/fifo-send.sh", "codex", PR_NUMBER, PROMPT])
