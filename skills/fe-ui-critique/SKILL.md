...
- 只有在用户提供 URL、route、截图或明确授权时，才做浏览器取证；登录态、私有页面和第三方站点默认按敏感 surface 处理。
- 截图、DOM snapshot、组件和 token 只用于视觉证据；不要复述或外传页面中的 secret、token、个人数据、内部链接或未公开业务数据。
- 如果取证需要认证、外部网络访问、敏感数据处理或超出视觉诊断的安全判断，先转 `/guard-secure`；涉及部署、远程或仓库外状态时转 `/guard-gitops`。
