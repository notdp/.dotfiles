...
多 agent 编排的长任务工作流。把"调度脑"从 agent 内部脑补迁到 YAML + 文件 SSOT：6 角色协作，phase coder tmux pane 跨 phase 长驻复用，review 一轮分级 + coder 自判 ack 收口，phase 间用户 confirm 介入。spec SSOT: `docs/specs/dev-long-run-v2/overview.md`。

harness: `skills/dev-long-run-v2/lr2.py`（薄 wrapper，纯逻辑可单测，多 pane 调度脑在 loop orchestrator LLM 里）。
...
## 回退
出问题时 `/dev-long-loop` 始终可用；删 worktree + 删分支 + 删 `skills/dev-long-run-v2/` `.long-loop/<ws>` 即完全回退，main 无残留。
