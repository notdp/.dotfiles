...
pane_name_for() {
  local target="$1"
  local index="${2:-0}"
  python3 - "$target" "$index" "${NOTIFY_TMUX_TITLE_PANE_NAMES:-}" <<'PY' 2>/dev/null || true
import sys, zlib
target, index_raw, custom = sys.argv[1], sys.argv[2], sys.argv[3]
names = custom.split() if custom.strip() else [
...
set_pane_name() {
  ...
  local index="${2:-0}"
  local pane_name
  pane_name="$(pane_option_name "$target")"
  if [[ -z "$pane_name" ]]; then
    pane_name="$(pane_name_for "$target" "$index")"
    [[ -z "$pane_name" ]] && return 0
    tmux set-option -qpt "$target" @notify_tmux_title_pane_name "$pane_name" 2>/dev/null || return 0
  fi
  tmux select-pane -t "$target" -T "$pane_name" 2>/dev/null || return 0
}
...
ensure_pane_names() {
  ...
  while IFS=$'\t' read -r pane index; do
    [[ -n "$pane" ]] && set_pane_name "$pane" "$index"
