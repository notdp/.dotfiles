...
来源吸收：`mattpocock/skills` 的 `triage/AGENT-BRIEF.md`（Durability over precision 原则）。

固定输出表格中的 `file:line` 引用适用于短期产物（review finding、verify 结果、diff scan），因为这些产物的生命周期不超过当次任务。

持久化产物（issue、PRD、agent brief、long task scaffold、handoff 文档）不应依赖文件路径和行号——它们会快速腐烂。持久化产物优先写：

- **Current behavior**：当前系统实际做什么
- **Desired behavior**：期望系统做什么
- **Key interfaces**：涉及哪些接口和行为契约
- **Acceptance criteria**：怎么判断完成
- **Out of scope**：明确不做什么

| 产物类型 | `file:line` | 行为契约 |
|---|---|---|
| review finding / verify 结果 / diff scan | 适用 | 可选 |
| issue / PRD / agent brief / handoff | 避免 | 必须 |
...
## 模式 M：指令-知识分离（XML tag 两层结构）

来源吸收：`mattpocock/skills` 的 `grill-with-docs`、`writing-beats`、`writing-fragments`。

用 XML tag 把 skill 正文拆成两层：agent 优先执行 `<what-to-do>` 中的流程，遇到细节时查 `<supporting-info>`。

### 用它的场景

- 工作流型 skill（60+ 行），指令和判断标准混在一起
- agent 经常先读完所有知识段再开始执行，导致执行路径不清晰
- 需要让 agent 区分"必须做的步骤"和"按需查阅的参考"

### 不适用

- 动作型 skill（<40 行），结构已经清晰
- 全文都是指令、没有辅助知识的 skill
...
<supporting-info>
## 术语定义
- **Term A**: 定义。_Avoid_: 别名 1、别名 2。

## 格式规范
- 输出必须包含 X、Y、Z 字段。

## 边界规则
- 遇到 <condition> 时，走 <alternative path>。
</supporting-info>
```

### 写法规则

- `<what-to-do>` 只放执行流程和硬门控，不放长解释
- `<supporting-info>` 放术语、格式、启发式、示例和低频规则
- 两段的信息不重复；`<what-to-do>` 可引用 `<supporting-info>` 中的术语
- 如果 `<supporting-info>` 也超过 80 行，考虑拆到支撑文件（模式 G）

### 在本仓库的映射

- 后续大改长 skill（如 `fe-ui-design`、`guard-review`、`dev-large-delivery`）时可选用
- 不要求回溯改造现有 skill

---

## 模式 N：.out-of-scope 知识库

来源吸收：`mattpocock/skills` 的 `.out-of-scope/` 目录 + `triage` skill 自动检查。

把被拒绝的 enhancement 持久化为文件，记录"不做什么"和"为什么不做"。triage / scope 决策时先查这个目录，相似需求可以引用历史决策而不是从头讨论。

### 用它的场景

- 长期维护的项目，同一个 enhancement 被反复提出
- 需要记住"为什么不做 X"的理由，避免 scope creep 和重复讨论

### 不适用

- 短期项目或一次性任务
- Bug（bug 应该被修，不是被 out-of-scope）
- 只出现一次、没有形成模式的请求
...
## 相关决策
<如果有的话，引用 ADR 或 commit>
```

### 写法规则

- 按 concept 组织，不按 issue 编号组织（concept 稳定，issue 编号腐烂）
- 只记录重复出现、已裁决的 enhancement
- 不默认创建 `.out-of-scope/` 目录；只在第一次裁决"不做且可能被再次提出"时 lazy create
- triage / scope 决策时先检查已有 out-of-scope 文件

### 在本仓库的映射

- 可在项目层面采用 `docs/out-of-scope/` 或 `.out-of-scope/`
- `/guard-close` 可在 scope 裁决后建议记录 out-of-scope（候选，不默认写文件）
- 本仓库已有 `scripts/fixtures/deprecated-concepts.json` 管理废弃概念，.out-of-scope 管理"不做的 enhancement"，两者不重叠

---

## 模式 O：单入口多模具分流（Mold Dispatch）

来源吸收：`refs/lijigang/ljg-skills` 的 `ljg-card`——单个 skill 用参数（`-l` 长图 / `-i` 信息图 / `-m` 多卡 / `-v` 视觉笔记 / `-c` 漫画 / `-w` 白板 / `-b` 大字）选择"模具"，每个模具对应一个 `references/mode-*.md`，触发时只加载选中模具的细节。

一类能力有多种执行模式时，用"同入口 + 参数路由 + 按模具加载 reference"，而不是拆成 N 个独立 skill。模具之间行为分离，但共享入口、命名和触发语义。

### 用它的场景

- 一个能力域有 3+ 种产出形态 / 执行模式，但触发意图相同（用户想"做这件事"，只是形态不同）
- 模式之间共享大量上下文（输入获取、命名、校验），只在最后一步分叉
- 拆成 N 个独立 skill 会让 description 互相打架、catalog 膨胀、近邻区分度变差（参 `docs/harness-governance-prd-2026-06-23.md` 的 A3）
...
---
name: foo
description: "...用 -a/-b/-c 选择模式... Use when ..."
---
## 参数
| 参数 | 模具 | 说明 |
|------|------|------|
| `-a`（默认） | ... | ... |
| `-b` | ... | 细节见 references/mode-b.md |
## 共享基础
<输入获取 / 命名 / 校验：所有模具公用>
## 分流
按参数加载 references/mode-{x}.md，执行对应模具
```

### 本仓库已隐式在用（本条把它显式化，供新 skill 编写参照）

- `guard-check`：按 diff 风险路由 review / secure / verify / ship / mysql-review
- `guard-review`：`--deep` / `--two-pass` 模式
- `fe-ui-*`：design / critique / lint / visual-iterate 同域多形态

### 护栏（关键）

- **参数 ≤ 5**：超过说明是多个能力硬塞在一起，应拆 skill（`ljg-card` 自己 7 个模具已偏多，是反面提醒；与"反模式 4：`{{var}}` 用太多"同源）
- 模具间必须行为分离、可独立验证；共享部分提到 `## 共享基础`，不在各模具里重复
- 每个模具细节下沉到 `references/mode-*.md`（叠加模式 G Progressive Disclosure），触发时只加载选中模具
- 与模式 I（Command 编排）区分：模式 I 是"串多个 skill"，本模式是"一个 skill 内多形态分流"

### 落地判据

- 多形态共享 ≥ 50% 上下文 → mold dispatch；各形态有独立调研 / 验证路径 → 仍拆独立 skill
- 本模式是**收敛**（少建 skill），与 governance "净新增校验 A2" 不冲突——它减少 description 互撞，方向与"近邻区分度"一致
...
| M 指令-知识分离 | mattpocock `grill-with-docs` | 混入 | 长 skill 指令和判断标准混杂，用 XML tag 分层 |
| N .out-of-scope 知识库 | mattpocock `.out-of-scope/` | 混入 | 持久化"不做什么"，防 scope creep 重复讨论 |
| O 单入口多模具分流 | `ljg-card` 的 `-l`/`-i`/`-m`… | 混入 | 一类能力多形态、共享上下文，避免拆 N 个 skill（参数 ≤ 5） |
