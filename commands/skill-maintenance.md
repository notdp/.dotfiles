...
4. 盘点本地资产：
   - `skills/catalog.json` 和 `skills/*/SKILL.md`
   - `commands/*.md`
   - `agents/AGENTS.md`
...
6. 准备前置 review packet，内容只包括证据源，不包括主命令预设候选结论：
   ...
   - 所有 `skills/*/SKILL.md`、`commands/*.md`、`agents/AGENTS.md`、`scripts/hooks/*.py` 的文件清单
...
7. 自动并行派发两个只读 reviewer：
   ...
   - Reviewer B default: agent `skill-maintenance-reviewer-b`, requested model `cliproxy/claude-opus-4-7`
...
10. **逐 ref 派发 subagent 调研**：对每个有新 commit 的 ref，派发一个只读 subagent（可并行），任务是：
   ...
   - 猜测该项目做这些变更的目的是什么（解决什么问题、追随什么趋势、修复什么缺陷）。
...
| B | `skill-maintenance-reviewer-b` | `cliproxy/claude-opus-4-7` | `high` | 独立逐一审查，重点找 A 可能漏掉的问题 |
