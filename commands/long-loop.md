...
- `help` 不得创建 `.long-loop/`，不得调用 `dev-long-loop`，不得根据上下文生成计划。
- `status` / `tail` / `watch` 只读，不得创建或修改 workspace。
- `plan` / `init` 只生成审批前 workspace，状态必须是 `awaiting_approval` + `approval=pending`。
- `run` / `resume` 只有在 `approval=approved` 后才能执行。
...
- 每轮结束必须输出阶段总结，并写入 `.long-loop/progress.md`、`events.jsonl` 与 append-only log。
...
| `plan` / `init` | yes | no | 生成计划并等待审批 |
| `approve` | yes | no | 显式批准计划 |
| `run` / `resume` | yes | yes | 执行 bounded iterations |
