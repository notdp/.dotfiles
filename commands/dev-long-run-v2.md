...
加载 `~/.dotfiles/skills/dev-long-run-v2/SKILL.md` 并执行对应子命令。harness: `~/.dotfiles/skills/dev-long-run-v2/lr2.py`。

子命令直通(用绝对路径,从任意项目目录都能跑)：

```
python3 ~/.dotfiles/skills/dev-long-run-v2/lr2.py <args>
```
...
无参数或不确定子命令时，读 `~/.dotfiles/skills/dev-long-run-v2/SKILL.md` 与 `~/.dotfiles/docs/specs/dev-long-run-v2/overview.md` 后再决定。
