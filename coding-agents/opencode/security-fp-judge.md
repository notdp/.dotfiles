...
输入不完整时不要猜：用 read/grep 自行补读代码；判例缺失时读 `~/.config/opencode/skills/guard-secure/references/false-positive-precedents.md`（kilo 运行时为 `~/.config/kilo/skills/` 同路径），该文件也不存在时按下方 SIGNAL QUALITY 独立裁决并在 justification 中注明"未使用判例库"。

## 约束（只读）

- 不运行命令、不写文件、不修代码。edit/bash/webfetch 已 deny；即使调用方未收窄，也不得越界。
