...
为什么用 PIEV：agent 修改代码最常见的失败不是“不够聪明”，而是改动对象不可预测、影响面不隔离、规则不显式、验证回路不存在。PIEV 把“好不好改”转成四类可观察失败模式，避免把个人风格偏好误判为结构问题。

### PIEV

| 维度 | 检查点 | 为什么 |
|------|--------|--------|
| **Predictable** | 模式稳定，行为与改法容易预测 | 避免 agent 靠猜测续写，导致同类逻辑出现多种写法 |
| **Isolated** | 改动局部，影响面可控 | 避免一个小需求扩散到远端调用者、全局状态或隐式副作用 |
| **Explicit** | 规则、约束、边界是显式的 | 避免 tacit knowledge 藏在调用顺序、注释或历史习惯里 |
| **Verifiable** | 改完有办法快速验证 | 避免只能靠“看起来合理”交付，缺少可重复证据 |

### 六个检查维度

| 维度 | 检查点 | 为什么 |
|------|--------|--------|
| **Boundary** | 职责、分层、输入输出、side effect 边界是否清晰 | 边界不清时，修复会穿透层级并制造隐藏耦合 |
| **Locality** | 一个需求是否主要集中在少数几个文件内完成 | 局部性差会让 review、测试和回滚成本快速上升 |
| **Convention** | 是否存在稳定、统一、可续写的实现模式 | 没有约定时，agent 容易新增平行体系 |
| **Explicitness** | 业务规则、约束、前提是否写出来，而不是藏在 tacit knowledge 里 | 隐式规则无法被工具搜索、测试或 review 稳定捕获 |
| **Testability** | 是否容易写单测/集成测试/contract test，验证回路是否短 | 不可测试的结构会把正确性判断推给人工猜测 |
| **Diff Purity** | 变更是否单一目的；机械改动、重构、行为修改是否混在一起 | 混杂 diff 会掩盖真实行为变化，增加回归定位难度 |
