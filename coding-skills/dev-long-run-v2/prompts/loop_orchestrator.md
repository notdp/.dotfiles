...
6. **完成门禁(L23,硬门 —— 不许跳、绝不手翻 `fix_plan.md`)**：
   ...
   b. `lr2.py complete-phase --workspace <ws> --phase <id>` —— 内部跑门禁(verify.json.ok=真 且 review 每个 `[blocker]` 在 ack 标 `[fixed]`)，**过了才翻 `fix_plan.md [x]`**；exit 2 按它打印的 BLOCK 原因打回，别绕过。门禁过后**自动 teardown 兜底**：关掉本 phase 残留的 planner/reviewer pane（coder 跨 phase 复用，不关）——所以 step 2/4 的显式 `close` 漏了也有兜底。
...
- 「完成 / 收尾 / done」 → 收尾(L23 收尾门禁)：① 写根 `acceptance.sh`(端到端自动化验收)→ ② `lr2.py verify --workspace <ws> --acceptance` 真跑、写 `acceptance.json` → ③ 把根 `qa.md` 的 `## 人工验证`(若有)贴给用户点验确认 → ④ `lr2.py complete-run --workspace <ws>`(acceptance 门禁过了才置 `state=completed`，否则拒绝；过了会**自动 teardown 所有 worker pane（含跨 phase 的 coder）**，要保留现场调试加 `--keep-panes`)→ ⑤ 整体扫 `BACKLOG.md` 标可快速收敛项、写 `CLEANUP_PROPOSAL.md`（只建议不删，L12/L15）。**没过 acceptance 门禁不准说"完成"。**
