...
4. **先 `lr2.py reset-status --phase NN --role phase_coder`**(清掉残留的 `done impl`,防 stale done 误判)→ `lr2.py send --pane <coder> --text "<review>" --workspace <ws>` → coder 写 `ack.md`(含 `## Blocker Resolutions`,每行 `- [fixed] <ID> …` 带 review 的 blocker ID)逐项 ack + 修 + 写 `verify.sh` → **commit 本 phase 到分支(L14)** → `await` 等到 **`DONE commit=<hash>`**。
