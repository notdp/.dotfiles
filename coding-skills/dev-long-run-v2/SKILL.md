...
- **L23(完成门禁)**: phase 完成 / run 收尾**只能经 `lr2.py complete-phase` / `complete-run` 过门禁**,不许手翻 `fix_plan.md`、不许手置 `state=completed`。门禁三条:① `verify.sh` 必须由 `lr2.py verify` **真跑**且过(verify.json.ok=真,测试写了不跑=没写);② review 每个 `[blocker]` 必须在 ack 标 `[fixed]`(不许 deferred);③ 收尾必须 `acceptance.sh` 真跑过 + 人工验收项经用户确认。**首次实战(00167)就栽在"验证只写没跑 + blocker 静默放行 + acceptance 没执行",L23 就是堵这个。**
