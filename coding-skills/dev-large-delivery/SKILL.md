...
| 目标明确、可拆 todo、需要多轮循环执行 | `/dev-long-loop` |
...
`/dev-long-loop` 是长任务工作流：用 `.long-loop/` 保存 prompt、spec overview、fix plan、qa 和 logs，让 agent 在预算内一轮一个 todo 地推进。

本 skill 是交付治理：定义 Phase 出口、test 等价演练、人工验收、回滚剧本和 prod 灰度。大型交付可以在某个 Phase 内使用 long-loop 做多轮实现，但 Phase 是否完成仍由本 skill 的 checklist 裁决。
