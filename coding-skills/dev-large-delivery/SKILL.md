...
| 目标明确、可拆 phase、需要多轮循环执行 | `/dev-long-run` |
...
`/dev-long-run` 是长任务执行工作流：用 `.long-loop/` 工作区保存 spec overview、fix plan、qa 和 logs，由对话 agent 调度 planner/coder/reviewer 逐 phase 实现、commit 并过完成门禁。
