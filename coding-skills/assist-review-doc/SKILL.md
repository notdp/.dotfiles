...
详细 schema、commit message 格式、heading 改名规则、role 字段语义 → `${HOME}/.dotfiles/coding-skills/assist-review-doc/CONVENTIONS.md`

Git commit 只在用户明确批准时执行。未获批准时，本 skill 只产出可提交的 `doc.md` / `comments.json` 状态、验证证据和建议 commit message。
...
```bash
python3 "${HOME}/.dotfiles/scripts/review_doc_render.py" --doc <doc.md> --output <doc>.review.html
# 写空 comments.json
# 如果用户明确批准本轮 commit，再执行：
...
```
1. 跑 ${HOME}/.dotfiles/scripts/review_doc_consume.py diff --baseline <git.json> --incoming <download.json>
...
4. 跑 ${HOME}/.dotfiles/scripts/review_doc_ids.py verify <doc.md> <comments.json>  # 校验无 orphan anchor
5. 输出建议 commit message："[review:v{N+1}] <doc-stem>: {M}blocker {K}question {J}nit {I}idea→backlog"；只有用户明确批准 commit 时才执行 git commit
...
| 脚本 | 作用 |
|---|---|
| `${HOME}/.dotfiles/scripts/review_doc_ids.py` | anchor ID 生成 (`extract`) + 稳定性校验 (`verify`)；orphan 评论时 exit 1 |
| `${HOME}/.dotfiles/scripts/review_doc_render.py` | MD → 评审 HTML，载入 comments.json 预填、textarea + sessionStorage、导出按钮 |
| `${HOME}/.dotfiles/scripts/review_doc_consume.py` | `diff` 提取新评论 + apply_resolution 库函数；版本必须单调递增 |
| `${HOME}/.dotfiles/scripts/review_doc_migrate.py` | schema v1 验证 + 未来 vN→vN+1 迁移入口 |

这些 review_doc helper 来自 dotfiles；不要假设目标项目有同名 review_doc 脚本。
...
- doc.md + comments.json 处于可提交状态；如用户明确批准 commit，则最新 `[review:v{N}]` commit 已落地
