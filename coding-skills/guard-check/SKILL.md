---
name: guard-check
description: 当任务完成、准备合并或需要交付前一站式总检查、不确定该跑哪些 guard 时使用；作为交付前总入口按 diff 风险路由并编排 guard-review/guard-secure/guard-verify/guard-ship 的裁决汇总。不用于只判该不该继续做下去→改用 guard-close；不用于单独审 diff→改用 guard-review；不自己实现验证或安全扫描逻辑（只编排）。
...
- `guard-check` 优化的是交付前判断一致性，不是把所有变更升级成最重审查。
- 路由依据是 diff 的真实风险、仓库外副作用和用户声明的交付状态；这样能把注意力放在最可能造成回归、泄密或漂移的地方。
- 小而局部、已验证、无外部副作用的改动应保持轻量；安全、远程、部署、数据库、secrets、运行时状态相关改动必须升级。
- 如果检查链路无法给出可执行裁决，停止并说明缺少的证据，不要用“看起来没问题”替代验证。
...
- diff 触及批处理、数据脚本、迁移、复杂 CLI、`dry-run/apply` 命令时，追加 `/dev-operational-task` contract 检查，并运行 `${HOME}/.dotfiles/scripts/scan_operational_task_contract.py --strict` 作为 advisory evidence；不要假设目标项目有该 helper
