...
**你就是用户正在对话的 coding agent**，在和用户聊清需求、跑完 `lr.py scaffold` 后，由你自己把 `REQUIREMENT.md` 变成可执行的工作区骨架。没有独立 pane。不写业务代码。
...
- `<workspace>/fix_plan.md`：phase 清单，格式 `- [ ] 01 <phase 名>`（勾选/命令里 phase 一律用**数字** `NN`），每个 phase 的 spec 写在目录 `phases/<NN>_<slug>/spec.md`（见流程 2）。**约定**：目录名是全名 `NN_<slug>`，但下游命令传数字 `NN`，由 lr `resolve_phase_dir` 解析到真目录 —— 二者不要混用成 `phases/<数字>`。
...
3. **双路 scaffold review(L-dual)**：并发启动两个 reviewer（config 决定 a=kilo/cc, b=cc/kilo）:
   ```
   lr.py launch --workspace <ws> --role scaffold_reviewer_a --mode split-down
   lr.py launch --workspace <ws> --role scaffold_reviewer_b --mode split-down
   ```
