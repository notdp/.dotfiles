...
- [ ] 若含单向门决策（标识符/存储/schema/envelope/鉴权/协议），已按 §6 One-way-door 发散摊出 ≥2 替代 + 各自反超条件；真正不可逆的已跑 `design-divergence-challenger` 并处理其 winning_condition（无单向门则不适用）
...
### One-way-door 默认值发散（不可逆决策强制摊替代）

反模式：对一个**单向门**决策顺手取通识默认（典型：主键直接用 UUID v4），它能过 review、过测试，恰恰是因为审查者、测试和作者共享同一个默认——隐患（索引局部性差、无法顺序/范围检索、写入热点）小系统不显，规模上来才爆。Premise Collapse 治"脆弱假设"，本节治"自信默认"：默认本身可能就是坑。

**触发：** 决策属于单向门——标识符/主键方案、存储引擎或核心 schema 形状、API response envelope、鉴权/权限模型、数据格式或线协议。迁移成本高 = 单向门。可逆决策跳过本节。

**要求：** 锁定前，对该默认摊出 **2 个最强替代**，并给出**每个在什么具体条件下反超你的选择**（工作负载、规模、访问模式、顺序/范围检索需求、索引局部性、失败模式、迁移成本）。给不出反超条件 = 没发散，是在合理化；形式可借上文 Approach 表的"放弃了什么备选、为什么"。

**真正不可逆的，不许自评：** 调 `design-divergence-challenger` subagent——它从一个**未被你的默认锚定**的上下文反驳你的默认、给出最强替代与反超条件；处理它返回的 `winning_condition`（确认不适用本系统，或改方案）再锁。这是 soft 纪律，无脚本强制；靠 agent 自觉 + review 抽查 spec 里是否记了替代权衡。

举例：`主键用 UUID v4` → 替代 `bigint 自增`（反超条件：单调写入、需顺序与范围扫描、在意索引局部性与体积）、`ULID`（反超条件：需分布式生成 *且* 需时间有序）→ 若系统有时间有序检索需求，默认 UUID 就该被挑战。
