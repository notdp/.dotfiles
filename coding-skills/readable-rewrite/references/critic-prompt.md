...
2. 读 `current`，按 rubric 四维度扫描：
   - **structure**：对象清单是否完整、是否横向铺表作主结构、节序、节内主题、否定式信息（"默认不做 X"、"不支持 Y"）是否被放在节开头或显著位置（除非是必须前置的危险警告）
   - **slot**：5 槽位完整性、机制是否含任一显式形式（伪代码 / I/O 示例 / mermaid 图 / 决策表）、边界四子项是否齐、首次出现的陌生术语 / 工具内部概念（如 `transcript_path`、`additionalContext`、`hookSpecificOutput`）是否给了一句白话解释或前置定义
   - **fidelity**：对每个对象**逐 slot** 检查（避免漏报）：
     ...
     - 行为/可靠性断言（"会"、"必然"、"稳定"、"保证"、"不变"、"一定"）是否标了 [推断]/[未验证]/[猜测] 标签或注明证据来源；未标 → P1
