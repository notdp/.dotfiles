---
name: think-research
description: 当实现前需要技术选型、方案对比、可行性评估或最佳实践调研，且要落地为带推荐和理由的结论时使用；产出证据地图+推荐方案+方案矩阵+Plan Handoff。不用于开放主题只汇总现状不下结论（改用 /think-survey），也不用于候选已明确只需按维度逐项打分裁决（改用 /think-compare）。
